-- Keep live status aligned with kickoff changes and prevent future matches from affecting live ranking.
-- If an admin moves an in-progress test match back to the future, the match returns to scheduled.

create or replace function private.reset_future_live_match_status()
returns trigger
language plpgsql
security definer
...
as $function$
begin
  if new.status = 'live' and new.kickoff_at > now() then
    new.status := 'scheduled';
    new.home_score := 0;
    new.away_score := 0;
    new.regulation_home_score := null;
    new.regulation_away_score := null;
    new.qualified_team_id := null;
    new.qualification_method := null;
...
drop trigger if exists reset_future_live_match_status_before_write on public.matches;

create trigger reset_future_live_match_status_before_write
before insert or update of status, kickoff_at on public.matches
for each row
execute function private.reset_future_live_match_status();
...
with active_match as (
  ...
  where deleted_at is null
    and (
      status = 'finished'
      or (status = 'live' and kickoff_at <= now())
    )
